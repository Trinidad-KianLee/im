    ...
    if (empty($student_id)) {
        ...
        if ($check_result->num_rows == 0) {
            ...
            if (!$stmt->execute()) {
                ...
                $stmt = $conn->prepare("INSERT INTO grade_level (Grade_level_ID, Student_ID, Year_level) VALUES (?, ?, 'kindergarten')");
                if ($stmt === false) {
                    die('Prepare failed: ' . htmlspecialchars($conn->error));
                }
...
<head>
    ...
    <title>Add Kindergarten Student - Mary Angel Learning Center</title>
